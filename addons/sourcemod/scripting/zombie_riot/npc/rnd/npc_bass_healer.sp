...
static const char g_BassBoostSound[][] = {
	"items/taunts/scout_boston_breakdance/scout_boston_breakdance_04.mp3",
};

static float RND_BassBooster_NextBoost[MAXENTITIES];
static float RND_BassBooster_NextMusic[MAXENTITIES];

void RND_BassHealer_OnMapStart_NPC()
{
...
methodmap RND_BassHealer < CClotBody
{
	...
	public RND_BassHealer(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		RND_BassHealer npc = view_as<RND_BassHealer>(CClotBody(vecPos, vecAng, "models/player/scout.mdl", "1.0", "800", ally));
		
		i_NpcInternalId[npc.index] = RND_BASS_HEALER;
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, RND_BassHealer_ClotDamaged);
		SDKHook(npc.index, SDKHook_Think, RND_BassHealer_ClotThink);
		...
		npc.m_iWearable4 = npc.EquipItem("head", "models/player/items/scout/boombox.mdl");
		SetVariantString("1.0");
...
//TODO 
//Rewrite
public void RND_BassHealer_ClotThink(int iNPC)
{
	RND_BassHealer npc = view_as<RND_BassHealer>(iNPC);
	...
	if (npc.m_fNextBassBoost <= GetGameTime(npc.index) && !NpcStats_IsEnemySilenced(npc.index))
	{
		...
		spawnRing(npc.index, 316.0*2, 0.0, 0.0, 1.0, "materials/sprites/laserbeam.vmt", 153, 153, 0, 200, 1, 0.1, 3.0, 3.1, 1);
		...
		for(int entitycount; entitycount<MAXENTITIES; entitycount++) //Check for npcs
		{
			if(IsValidEntity(entitycount) && entitycount != npc.index && (entitycount <= MaxClients || !b_NpcHasDied[entitycount])) //Cannot buff self like this.
			{
				if(GetEntProp(entitycount, Prop_Data, "m_iTeamNum") == GetEntProp(npc.index, Prop_Data, "m_iTeamNum") && IsEntityAlive(entitycount))
				{
					...
					{
						if(i_NpcInternalId[entitycount] != RND_BASS_HEALER) //Hussars cannot buff eachother.
						{
							int currentHp = GetEntProp(entitycount, Prop_Data, "m_iHealth") + 1;
							if(currentHp >= GetEntProp(entitycount, Prop_Data, "m_iMaxHealth"))
							{
								currentHp = GetEntProp(entitycount, Prop_Data, "m_iMaxHealth");
							}
							SetEntProp(entitycount, Prop_Data, "m_iHealth", currentHp);
						}
					}
				}
			}
...
public Action RND_BassHealer_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	RND_BassHealer npc = view_as<RND_BassHealer>(victim);
...
public void RND_BassHealer_NPCDeath(int entity)
{
	RND_BassHealer npc = view_as<RND_BassHealer>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
	
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, RND_BassHealer_ClotDamaged);
	SDKUnhook(npc.index, SDKHook_Think, RND_BassHealer_ClotThink);
