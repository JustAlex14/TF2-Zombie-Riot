...
static const char g_RageSounds[][] = {
	"vo/mvm/mght/heavy_mvm_m_revenge01.mp3",
	"vo/mvm/mght/heavy_mvm_m_revenge02.mp3",
	"vo/mvm/mght/heavy_mvm_m_revenge03.mp3",
	"vo/mvm/mght/heavy_mvm_m_revenge04.mp3",
	"vo/mvm/mght/heavy_mvm_m_revenge05.mp3",
	"vo/mvm/mght/heavy_mvm_m_revenge06.mp3",
	"vo/mvm/mght/heavy_mvm_m_revenge07.mp3",
	"vo/mvm/mght/heavy_mvm_m_revenge08.mp3",
};

void RND_Heavy_OnMapStart_NPC()
{
...
methodmap RND_Heavy < CClotBody
{
	...
	public void PlayRageSound() {
		...
		#if defined DEBUG_SOUND
		PrintToServer("CGoreFast::PlayRageSound()");
		#endif
	}
	
	public RND_Heavy(int client, float vecPos[3], float vecAng[3], bool ally)
	{
		RND_Heavy npc = view_as<RND_Heavy>(CClotBody(vecPos, vecAng, "models/player/heavy.mdl", "1.0", "5000", ally));
		
		i_NpcInternalId[npc.index] = RND_HEAVY;
		...
		SDKHook(npc.index, SDKHook_OnTakeDamage, RND_Heavy_ClotDamaged);
		SDKHook(npc.index, SDKHook_Think, RND_Heavy_ClotThink);		
...
//TODO 
//Rewrite
public void RND_Heavy_ClotThink(int iNPC)
{
	RND_Heavy npc = view_as<RND_Heavy>(iNPC);
...
public Action RND_Heavy_ClotDamaged(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	RND_Heavy npc = view_as<RND_Heavy>(victim);
	...
	if (!npc.Anger)
	{
		if ((GetEntProp(npc.index, Prop_Data, "m_iMaxHealth")/2) >= GetEntProp(npc.index, Prop_Data, "m_iHealth")-damage)
		{
			npc.Anger = true;
			npc.PlayRageSound();
...
public void RND_Heavy_NPCDeath(int entity)
{
	RND_Heavy npc = view_as<RND_Heavy>(entity);
	if(!npc.m_bGib)
	{
		npc.PlayDeathSound();	
	}
	
	SDKUnhook(npc.index, SDKHook_OnTakeDamage, RND_Heavy_ClotDamaged);
	SDKUnhook(npc.index, SDKHook_Think, RND_Heavy_ClotThink);	
